...
class BaseL0Layer(nn.Module):
    ...
    def __init__(
        ...
        if droprate_init <= 0.0 or droprate_init >= 1.0:
            raise ValueError(f"expected droprate_init in (0,1). Got {droprate_init}")
        if temperature <= 0.0 or temperature >= 1.0:
            raise ValueError(f"expected temperature in (0,1). Got {temperature}")
    ...
    def get_parameters(self, weight_z: Tensor, bias_z: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        if weight_z is None:
