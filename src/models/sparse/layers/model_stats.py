...
from src.models.sparse import layers
...
class ModelStats:
    ...
    layer_stats: list[layers.LayerStats] = field(repr=False)

    num_params: int
    num_active_params: int | torch.Tensor

    num_sparse_params: int
    num_active_sparse_params: int | torch.Tensor

    sq_l2_norm: float | torch.Tensor | None = None

    num_gate_params: int | None = None

    def compute_density_stats(self) -> tuple[float | torch.Tensor, float | torch.Tensor | None]:
